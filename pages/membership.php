...
<section class="max-w-6xl mx-auto py-10 px-4">
  ...
  <div class="max-w-2xl mx-auto bg-white/40 backdrop-blur-sm rounded-3xl shadow-2xl overflow-hidden">
    ...
    <div class="w-full h-1.5 bg-slate-200">
      <div id="progress-bar" class="h-full bg-gradient-to-r from-amber-500 to-amber-400 transition-all duration-500" style="width: 25%;"></div>
    </div>
    ...
    <div class="p-8 min-h-[600px] flex flex-col">
      ...
      <form class="flex-1 flex flex-col" id="membership-form">
        ...
        <div id="step-2" class="form-step flex-1 hidden">
          <div class="space-y-6">
            ...
            <label class="block">
              ...
              <input type="text" name="institution" placeholder="Enter your school/university name" required
              />
            </label>
            <label class="block">
              <span class="text-sm font-semibold text-slate-700 mb-2 block">Field of Study</span>
              <input type="text" name="field_of_study" placeholder="Enter your major/field of study" required
              />
            </label>
          </div>
        </div>
          ...
          <div class="space-y-2">
            <fieldset>
              ...
              <div class="grid grid-cols-2">
                <label class="inline-flex items-center gap-2">
                  <input type="radio" name="heard_from" value="Social Media" required class="accent-blue-600">
      ...
      <label class="block">
        ...
        <div class="flex justify-between items-center mt-6">
          <button type="button" id="back-btn" class="hidden items-center justify-center gap-2 px-6 py-3 text-blue-600 font-semibold hover:text-blue-800 transition-colors">
            <?= inline_svg("assets/icons/arrow-left-solid-full.svg") ?> Back
          </button>
          <div id="back-spacer"></div>
          <button type="button" id="next-btn" class="inline-flex items-center gap-2 grad_secondary text-white px-8 py-3 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 font-semibold">
            Next <?= inline_svg("assets/icons/arrow-right-solid-full.svg") ?>
          </button>
          <button type="submit" id="submit-btn" class="hidden items-center gap-2 grad_primary px-8 py-3 rounded-xl shadow-lg hover:from-green-700 hover:to-green-800 hover:shadow-xl transform hover:scale-105 transition-all duration-200 font-semibold">
            <?= inline_svg("assets/icons/paper-plane-solid-full.svg") ?> Submit
          </button>
        </div>
      </form>
    </div>
...
function updateUI() {
  ...
  document.querySelectorAll('.form-step').forEach((el, idx) => el.classList.toggle('hidden', idx !== currentStep-1));
...
document.getElementById('next-btn').addEventListener('click', () => {
  ...
  if (!validateStep(stepEl)) { alert('⚠️ Please fill all required fields before proceeding.'); return; }
  if (currentStep < totalSteps) { currentStep++; updateUI(); }
});

// Back button
document.getElementById('back-btn').addEventListener('click', () => { if (currentStep>1){currentStep--;updateUI();} });

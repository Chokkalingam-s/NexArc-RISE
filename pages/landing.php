...
$stmt = $conn->prepare("SELECT * FROM Project WHERE status = 'Ongoing' ORDER BY year_of_start DESC LIMIT 6");
$stmt->execute();
$ongoingProjects = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch completed projects
$stmt = $conn->prepare("SELECT * FROM Project WHERE status = 'Completed' ORDER BY year_of_end DESC LIMIT 6");
$stmt->execute();
...
<section class="px-4 max-w-7xl mx-auto">
  <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-10 gap-4">
    ...
    <a href="/projects" class="group inline-flex items-center gap-2 px-5 py-2.5 bg-amber-500 text-amber-50 font-medium rounded-full transition-all duration-300 hover:scale-105 hover:bg-amber-200 shadow">
      View More
      <?php include __DIR__ . "/../assets/icons/right_arrow.svg"; ?>
    </a>
  </div>
      ...
      <?php foreach ($ongoingProjects as $project): ?>
        <div class="bg-white/50 rounded-2xl shadow-md hover:shadow-lg transition-all duration-500 hover:-translate-y-2">
          <div class="aspect-[4/2] rounded-t-2xl overflow-hidden">
            <img src="<?= htmlspecialchars($project['image'] ?: 'https://placehold.co/400x200') ?>"
          ...
          <div class="p-4">
            ...
            <p class="text-slate-700 text-sm mb-4 line-clamp-3">
              <?= htmlspecialchars($project['description'] ?: 'Revolutionary solution in development') ?>
            </p>
        ...
        <div class="relative overflow-hidden rounded-2xl bg-white/40 shadow-lg hover:shadow-xl transition-all duration-500 hover:-translate-y-2">
          ...
          <div class="p-4">
            ...
            <p class="text-slate-600 text-sm mb-4 line-clamp-3">
              <?= htmlspecialchars($project['description'] ?: 'Successfully delivered solution') ?>
            </p>
